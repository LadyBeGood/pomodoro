<script lang="ts">
    ...
    let tasks: Task[] = $state([
        {
            index: 1,
            task: "Get a job and escape unemployment",
            completed: false,
        },
        {
            index: 2,
            task: "Master Svelte 5 snippets and runes",
            completed: false,
        },
        { index: 3, task: "Complete the project", completed: false },
        {
            index: 4,
            task: "Polish portfolio and GitHub README",
            completed: true,
        },
        {
            index: 5,
            task: "Network with 3 industry professionals",
            completed: false,
        },
    ]);
    ...
    function addTask(event: Event) {
        ...
        tasks = [
            ...tasks,
            { index: nextIndex, task: newTask.trim(), completed: false },
        ];
        newTask = "";
...
<div
    class="bg-(--blackout) w-full text-(--luxury-white) overflow-y-auto px-8 pt-24 pb-36 select-none light-scrollbar h-full grid place-items-center"
>
    <div class="max-w-83.25 mx-auto flex flex-col relative">
        {#each tasks as task (task.index)}
            <div
                class="w-full overflow-x-auto flex no-scrollbar snap-x snap-mandatory items-center"
            >
                <button
                ...
                >
                    <p
                        class="text-lg font-medium leading-snug flex-1 transition-all
                        {task.completed
                            ? 'styled-line-through text-(--dravit-grey)'
                            : 'text-(--luxury-white)'}"
                    >
                        {task.task}
                    </p>
        ...
        <div class="flex gap-6 items-start py-4 relative">
            <input
                ...
                // to remove it.
                //
                // One solution was to use type="search" but that puts a "search" icon in the user's keyboard
                ...
                onkeydown={(event) => {
                    if (event.key === "Enter")
                        addTask(event), event.currentTarget.blur();
                }}
...
<style>
    .styled-line-through {
        ...
        background: repeating-linear-gradient(-45deg, transparent 0 8px, var(--dravit-grey) 0 11px, transparent 0) ;
        background: repeating-linear-gradient(-30deg, red 0 8px, var(--dravit-grey) 0 16px, red 0) ; */
        /* background: repeating-linear-gradient(-45deg, #3f87a6, #ebf8e1 15%, #f69d3c 20%); */
        /* background: linear-gradient(-45deg, transparent 45%, var(--dravit-grey) 0 55%, transparent 0)  0 / 18px 18px; 
