<script lang="ts">
    ...
    const focusDurationInSeconds = 1 * 60; // 25 minutes in seconds
    ...
    if (typeof window !== "undefined") {
        ...
        if (stored) {
            try {
                ...
                savedTime = parsed.timeLeft ?? focusDurationInSeconds;
                savedIsRunning = parsed.isRunning ?? false;
            } catch (e) {
                console.error("Failed to parse timer storage", e);
            }
        }
    }

    let timeLeft = $state(savedTime);
    let isRunning = $state(savedIsRunning);
    ...
    // Auto-save state to localStorage whenever values change
    $effect(() => {
        localStorage.setItem(localStorageKey, JSON.stringify({ timeLeft, isRunning }));
    });
    ...
    <div class="flex items-center justify-center shrink-0 flex-col gap-4 w-svw h-svh snap-start snap-always">
        ...
        <div class="relative flex justify-center gap-10">
            ...
            <button
                ...
                class="bg-luxury-white rounded-full p-3 mt-8"
