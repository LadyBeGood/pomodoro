...
<div class="shadow-2xl fixed bottom-5 left-1/2 -translate-x-1/2 bg-(--luxury-white) px-4 py-3 rounded-full flex gap-6 min-w-max text-(--luxury-white)">
    <a href={p("/statistics")} aria-label="Statistics page" class="rounded-full bg-(--blackout) p-2.5">
    ...
    <a href={p("/")} aria-label="Home page" class="rounded-full bg-(--blackout) p-2.5">
    ...
    <a href={p("/tasks")} aria-label="Tasks page" class="rounded-full bg-(--blackout) p-2.5">
